...
void youngest(struct Student studentArr[], int number) {
    ...
    for (int i = 1; i < number; i++) { // Skip the first student in the array
        if (studentArr[i].birthYear > studentArr[result].birthYear) // Greater birthYear -> Younger
            result = i;
    ...
    /* If the youngest student shares a birthday with another student in the array, result will store the value of the
	smallest index (the student who appear first in the array) */

    printf("\nYoungest student(s) (born on %d/%d/%d): %s (%s)",
